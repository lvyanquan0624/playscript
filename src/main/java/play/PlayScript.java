...
public class PlayScript {
    ...
    private static void REPL(boolean verbose, boolean ast_dump) {
        ...
        while (true) {
            try {
                ...
                scriptLet.append(line).append("\n");
                if (line.endsWith(";")) {

                    // 解析整个脚本文件
                    AnnotatedTree at = compiler.compile(script.toString() + scriptLet, verbose, ast_dump);
     ...
     * 运行指定名称的Java类
     */
    private static void runJavaClass(byte[] b){
        try {
            //java.lang.Class clazz = java.lang.Class.forName(className);
            java.lang.Class clazz = loadClass("DefaultPlayClass",b);
